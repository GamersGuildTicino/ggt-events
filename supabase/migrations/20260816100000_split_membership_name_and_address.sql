...
alter table public.memberships
add constraint memberships_first_name_not_blank
check (btrim(first_name) <> '')
not valid;

alter table public.memberships
add constraint memberships_last_name_not_blank
check (btrim(last_name) <> '')
not valid;

alter table public.memberships
add constraint memberships_street_not_blank
check (btrim(street) <> '')
not valid;

alter table public.memberships
add constraint memberships_postal_code_not_blank
check (btrim(postal_code) <> '')
not valid;

alter table public.memberships
add constraint memberships_city_not_blank
check (btrim(city) <> '')
not valid;
