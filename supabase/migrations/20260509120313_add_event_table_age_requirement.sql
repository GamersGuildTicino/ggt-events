create type "public"."event_table_age_requirement" as enum ('kids', 'age_14_plus', 'age_18_plus');

drop function if exists "public"."fetch_public_event_tables"(p_event_id uuid);

alter table "public"."event_tables" add column "age_requirement" public.event_table_age_requirement not null default 'age_14_plus'::public.event_table_age_requirement;

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.fetch_public_event_tables(p_event_id uuid)
 RETURNS TABLE(id uuid, time_slot_id uuid, game_system_id uuid, title text, description text, image_url text, game_master_name text, experience_level public.event_table_experience_level, age_requirement public.event_table_age_requirement, language public.event_table_language, notes text, min_players integer, max_players integer, created_by uuid, created_at timestamp with time zone, updated_at timestamp with time zone, registration_count integer)
 LANGUAGE sql
