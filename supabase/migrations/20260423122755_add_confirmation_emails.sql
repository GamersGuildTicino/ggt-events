drop policy "admins can delete registrations" on "public"."event_registrations";

drop policy "admins can insert registrations" on "public"."event_registrations";

drop policy "admins can update registrations" on "public"."event_registrations";

drop function if exists "public"."register_for_event_table"(p_event_table_id uuid, p_player_name text, p_email text);

alter table "public"."event_registrations" add column "locale" text not null default 'en-GB'::text;

alter table "public"."event_registrations" add constraint "event_registrations_locale_valid" CHECK ((locale = ANY (ARRAY['en-GB'::text, 'it-CH'::text]))) not valid;

alter table "public"."event_registrations" validate constraint "event_registrations_locale_valid";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.register_for_event_table(p_event_table_id uuid, p_player_name text, p_email text, p_locale text DEFAULT 'en-GB'::text)
