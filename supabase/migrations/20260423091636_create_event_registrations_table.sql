...
CREATE UNIQUE INDEX event_registrations_table_email_unique ON public.event_registrations USING btree (event_table_id, email);
...
alter table "public"."event_registrations" add constraint "event_registrations_email_normalized" CHECK ((email = lower(btrim(email)))) not valid;

alter table "public"."event_registrations" validate constraint "event_registrations_email_normalized";

alter table "public"."event_registrations" add constraint "event_registrations_email_not_blank" CHECK ((email <> ''::text)) not valid;

alter table "public"."event_registrations" validate constraint "event_registrations_email_not_blank";

alter table "public"."event_registrations" add constraint "event_registrations_event_table_id_fkey" FOREIGN KEY (event_table_id) REFERENCES public.event_tables(id) ON DELETE CASCADE not valid;

alter table "public"."event_registrations" validate constraint "event_registrations_event_table_id_fkey";

alter table "public"."event_registrations" add constraint "event_registrations_player_name_not_blank" CHECK ((btrim(player_name) <> ''::text)) not valid;

alter table "public"."event_registrations" validate constraint "event_registrations_player_name_not_blank";

alter table "public"."event_registrations" add constraint "event_registrations_table_email_unique" UNIQUE using index "event_registrations_table_email_unique";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.register_for_event_table(p_event_table_id uuid, p_player_name text, p_email text)
